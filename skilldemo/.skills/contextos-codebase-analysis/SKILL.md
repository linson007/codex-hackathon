...
description: Use ContextOS to create, update, query, and inspect local knowledge bases for multi-repo codebases, especially Spring Boot microservices. Trigger when the user asks what files to inspect before a change, impact analysis, impacted files/endpoints/tables/dependencies, codebase onboarding, service discovery, endpoint/entity/topic/table dependency questions, Spring service/repository/controller questions, spring-petclinic or retail-platform questions, or when a user asks to use ContextOS.
---
...
Use this command path for ContextOS on this machine:
...
If the user has globally installed ContextOS, `contextos` is also acceptable, but prefer the `CTX` path above for reliable demos from any folder.

When answering ContextOS-triggered questions, run commands. Do not answer only from general model knowledge.
...
If a shell does not retain `CTX`, use `/Users/linsonkurian/Documents/code/codex-hackathon/bin/contextos.js` directly.

## Workflow

1. Set `CTX=/Users/linsonkurian/Documents/code/codex-hackathon/bin/contextos.js`.
2. List existing KBs with `$CTX kbs`.
3. Reuse an existing KB when it matches the user’s target codebase.
4. If code may have changed, run `$CTX update <kb>` before answering.
5. For impact, dependency, or "files to inspect" questions, run `$CTX ask <kb> "<question>" --with-docs --verbose`.
6. Summarize the ContextOS output for the user, preserving impacted files, endpoints, tables, services/controllers/repositories, and caveats.
7. Generate or refresh onboarding docs with `$CTX docs generate <kb> --force` when the user wants richer repo/service/endpoint docs.
8. Start the UI only when the user asks for visual exploration: `$CTX ui`.
9. Stop the UI with `$CTX stopui` when done or when ports are stuck.
10. Use `$CTX export <kb> --format json` only when another tool needs a structured payload.

Mandatory command sequence for "what files should I inspect", "impact", or "dependencies" questions:

```bash
CTX=/Users/linsonkurian/Documents/code/codex-hackathon/bin/contextos.js
$CTX kbs
$CTX ask <kb> "<user question>" --with-docs --verbose
```

Choose `<kb>` by matching the user’s codebase name to the KB list. For `spring-petclinic`, use `spring-petclinic`. For the refund sample, use `retail-platform`.

<!--
Older generic workflow retained for humans:
...
- For a live Codex/Claude skill demo, read `references/agent-demo.md`.
